...
SELECT 
    ...
    oi.price,
    oi.freight_value,
    (oi.price + oi.freight_value) AS totalAmount
FROM orders_view o
    INNER JOIN order_items_view oi ON o.order_id = oi.order_id
    INNER JOIN customers_view c ON o.customer_id = c.customerId
...
SELECT 
    ...
    COUNT(DISTINCT f.order_id) AS orderCount,
...
SELECT 
    ...
    ROUND(SUM(f.totalAmount), 2) AS totalSalesAmount
FROM OLAP_FACTS_ORDER_AMOUNTS f
    INNER JOIN OLAP_DIM_SELLERS_GEO s ON f.seller_id = s.sellerId
